...
const Home:React.FC = ():JSX.Element => {
  ...
  const categories = useMemo(() => {
    return [...new Set(copyNewsData.current.map(item => item.category).filter(Boolean))]
  ...
  const sources = useMemo(() => {
    return [
      ...new Set(
        copyNewsData.current.map(item => item.source.name).filter(Boolean)
      )
    ].map(source => ({ value: source, label: source }));
  }, [copyNewsData]);
  
  const authors = useMemo(() => {
    return [...new Set(copyNewsData.current.map(item => item.author).filter(Boolean))]
      .map(author => ({ value: author, label: author }));
  }, [copyNewsData]);
console.log(categories,sources,authors,"autors")
  ...
  <section className="filter-list">
    <label>Filters:</label>
    {
      Object.keys(initialFilterValues).map((value: string): JSX.Element | string => {
        ...
        <FilterModal categories={categories} sources={sources} authors={authors} show={isFilterModalOpen} handleClose={handleCloseFilterModal} initialData={initialFilterValues} handleSubmit={handleSubmitFilterData} />
